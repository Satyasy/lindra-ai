...
Kartu bg `--danger`, radius lg, padding 22–24px. Judul kiri 700 (default **"Sedang dalam bahaya sekarang?"**). Kanan: 2 chip `tel:` — **Telpon SAPA 129 · Telpon Guru BK** (Polisi/Ambulans dihapus dari sisi siswa).
...
- **Follow-up proaktif — TERBANGUN.** Opt-in di layar konfirmasi (toggle DEFAULT OFF + consent verbatim §4.3), email disimpan terenkripsi di `Followup.contactEmail` (ciphertext base64 via identity-crypto). Cron `/api/cron/followup-email` (jadwal `FOLLOWUP_CRON_SCHEDULE`, Diagram A): report belum pernah dibuka + SLA breach → auto-escalate SAPA 129 sekali (`escalated`), BUKAN email; sudah dibuka → email NETRAL via **SMTP (nodemailer)** — env `SMTP_HOST/PORT/USER/PASS` + `FOLLOWUP_EMAIL_FROM`; tanpa `SMTP_HOST` di dev → `jsonTransport`/log (tanpa kode/link/token/kata "kekerasan"/"laporan"). **Timing demo:** email pertama ~1 menit setelah opt-in (`scheduledAt = now+60s`, cron tiap menit), bukan menunggu SLA. Input kode MANUAL `/followup/masuk` (tanpa kode di URL, tanpa auto-login) → sesi `/followup`. `noProgressCount` mencapai 3 → opsi SAPA 129 yang siswa pilih sendiri (tak otomatis) + sinyal informatif di detail BK. SLA scheduler penuh (reschedule kompleks) tetap ruang perbaikan.
